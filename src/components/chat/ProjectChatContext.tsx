...
export const ProjectChatContext = createContext<StreamResponse>({
...
export const ProjectChatContextProvider = ({ projectId, children }: Props) => {
    ...
    const { mutate: sendMessage } = useMutation({
        mutationFn: async ({ message }: { message: string }) => {
            const response = await fetch('/api/message/project', {
                method: 'POST',
                body: JSON.stringify({
                    projectId,
                    // fileIds: projectFiles,
                    message,
                })
            })
        ...
        onMutate: async ({ message }) => {
            ...
            const previousMessages = utils.getProjectMessages.getInfiniteData()

            // Step 3 - optimistically insert the new value. This is the endpoint we want to optimistically change the data for
            utils.getProjectMessages.setInfiniteData(
                { projectId, limit: INFINITE_QUERY_LIMIT },
        ...
        onError: (_, __, context) => {
            ...
            utils.getProjectMessages.setData(      //  Rollback to previous messages
                { projectId },
